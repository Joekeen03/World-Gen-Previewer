...
import main.java.ruby_phantasia.world_gen_previewer.lwjglBackend.Camera;
import main.java.ruby_phantasia.world_gen_previewer.lwjglBackend.TransformPipeline;
...
public class LWJGLBackend {
    ...
    private static final Path PRIMITIVE_VERTEX_SHADER_PATH = shadersDirectory.resolve("PrimitiveVertexShader.vert");
    ...
    private int SetupVertexBuffer(PooledVertices pooledVertices) {
        FloatBuffer vertexBuffer = memAllocFloat(pooledVertices.vertices.length* Vertex.N_FLOATS);
        for (Vertex vertex : pooledVertices.vertices) {
            ...
            vertexBuffer.put(vertex.color.w());
        }
        vertexBuffer.flip();
    ...
    private void Loop() {
        ...
        Primitive[] primitives = {
                new Cube(DefaultVectors.ZERO, 1.0f, new Vector3f(0.5f)),
                new Cube(new Vector3f(0.0f, 2.0f, 0.0f), 1.0f),
                new Cone(new Vector3f(0.0f, 1.0f, 0.0f), new Vector3f(1.0f, 1.0f, 1.0f), 1.0f, 2.0f, new Vector3f(1.0f, 0.0f, 1.0f)),
        };
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            glVertexAttribPointer(0, Constants.VECTOR3F_N_FLOATS, GL_FLOAT, false, Vertex.N_FLOATS_BYTES, Vertex.POSITION_OFFSET_BYTES);
            glVertexAttribPointer(1, Constants.VECTOR4F_N_FLOATS, GL_FLOAT, false, Vertex.N_FLOATS_BYTES, Vertex.COLOR_OFFSET_BYTES);
    ...
    private static class PooledVertices {
        ...
        public PooledVertices(Vertex[] vertices, int[] indices, int[] primitiveIndexStarts) {
        ...
        public static PooledVertices PoolVertices(Primitive[] primitives) {
            ...
            Vertex[] pooledVertices = Arrays.stream(primitives).flatMap(primitive -> {
                ObjectImmutableList<Vertex> primitiveVertices = primitive.GetVertices();
            ...
            }).toArray(length -> new Vertex[length]);
